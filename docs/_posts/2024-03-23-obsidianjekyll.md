...
- With `Files and links > New link format > Absolute path in vault`, [[obsidian/AdditionalNote|AdditionalNote]] shows as `[AdditionalNote](/additionalnote.md)` when entered Wiki-style, whereas [AdditionalNote](/subfolder/additionalnote.md) shows as `[AdditionalNote](/subfolder/additionalnote.md)` when entered Wiki-style.
